...
def mac_platforms(version=None, arch=None):
    ...
    if version is None:
        version = cast("MacVersion", tuple(
            map(int, version_str.split(".")[:2])))
    else:
        version = version
    if arch is None:
...
def _is_manylinux_compatible(name, arch, glibc_version):
    ...
    try:
        from auditwheel import _manylinux  # noqa
    except ImportError:
        pass
    else:
...
def _glibc_version_string_confstr():
    ...
    try:
        ...
        # type: ignore # type: Tuple[str, str]
...
def _manylinux_tags(linux, arch):
    ...
    for glibc_major in range(current_glibc.major - 1, 1, -1):
        glibc_max_list.append(glibcVersion(
            glibc_major, _LAST_GLIBC_MINOR[glibc_major]))
